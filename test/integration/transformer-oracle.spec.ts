...
describe.only('TransformerOracle', () => {
  ...
  before(async () => {
    ...
    await deployments.run(
      ['ChainlinkFeedRegistry', 'TransformerRegistry', 'ProtocolTokenWrapperTransformer', 'wstETHTransformer', 'TransformerOracle'],
      {
        resetMemory: true,
  ...
  describe('using the transformer oracle', async () => {
    when('using the wstETH transformer', () => {
      given(async () => {
        const wstETHTransformer = await ethers.getContract<ProtocolTokenWrapperTransformer>('wstETHTransformer');
